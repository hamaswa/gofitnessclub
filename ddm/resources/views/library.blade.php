        ...
        @foreach ($data as $item)
            <li class="library-input border bg-white rounded-top d-flex justify-content-between align-items-center mb-2 p-2">
                ...
                <a href="javascript:void(0)" data-value="{{ $item->name }} = {{ $item->weight }}, {{ $item->energy}}" 
                    id={{ $item->id }}  class=" py-0 px-2 btn-edit-item">Save
                </a>
                <a href="javascript:void(0)" data-value="{{ $item->name }} = {{ $item->weight }}, {{ $item->energy}}" 
                    id={{ $item->id }}  class=" py-0 px-2 btn-delete-item">Delete
                </a>
            </li>                      
        @endforeach    
    </ul>
   ...
   @endphp 
     ...
     <ul class="list-unstyled">
        <li class="library-input border bg-white rounded-top d-flex justify-content-between align-items-center mb-2 p-2"> 
           ...
           <a href="javascript:void(0)" class=" py-0 px-2" id='btn-add-item'>Add</a>
        </li>
     </ul>   
